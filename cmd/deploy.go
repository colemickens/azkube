...
func NewDeployCmd() *cobra.Command {
	...
	flags.String("flavor", "coreos", "the flavor of deployment to perform (currently supported: coreos, coreos-ex)")
...
func parseDeployArgs(cmd *cobra.Command, args []string) (RootArguments, DeployArguments) {
	...
	if deployArgs.Flavor == "coreos-ex" && deployArgs.NoCloudProvider == true {
		log.Fatalf("--no-cloud-provider is disallowed with --flavor=coreos-ex. This flavor requires a ServicePrincipal to use for the CloudProvider.")
	}
